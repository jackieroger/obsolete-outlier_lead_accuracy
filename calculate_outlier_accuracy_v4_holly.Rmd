...
cohort_names <- c("pancan_thresholds", "nof1_disease_thresholds", "pandis_thresholds", "first_degree_thresholds", "first_and_second_degree_thresholds")
this_gene <- "PARP1"
thresholds<-lapply(cohort_names, function(x) {
  f1<-threshold_info[[x]]
  this_threshold <- f1$high[this_gene]
  if(is.null(this_threshold)) this_threshold <- NA
  return(this_threshold)
}
  ) %>% unlist %>% as.numeric

this_expr <- outlier_info %>% filter(Gene == this_gene) %>% pull(sample)


threshold_tib <- tibble(
  sample = sample_id,
  gene = this_gene,
  cohort = cohort_names, 
