...
compute_avg <- function(tib) {
  if (!(is.null(tib))) {
    avg <- mean(tib$pct_accurately_measured)
    return (avg)
  }
}

compute_min <- function(tib) {
  if (!(is.null(tib))) {
    min <- min(tib$pct_accurately_measured)
    return (min)
  }
}

compute_max <- function(tib) {
  if (!(is.null(tib))) {
    max <- max(tib$pct_accurately_measured)
    return (max)
  }
}
...
for (i in 1:length(outlier_leads)) {
  ...
  summary <- summary %>%
    mutate(exceeds_threshold = (expression > threshold),
           percent_difference = abs(expression - threshold))
  ...
  summary <- summary %>%
    mutate(avg = lapply(measurement_stats_filtered, compute_avg),
           min = lapply(measurement_stats_filtered, compute_min),
           max = lapply(measurement_stats_filtered, compute_max),
           S1 = NA,
           S2 = NA,
           S3 = NA,
           S4 = NA,
